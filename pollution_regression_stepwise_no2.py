...
data = result.drop(columns = ['O3','一氧化氮','一氧化碳','二氧化硫','氮氧化物','PM2.5','年','月','日','点位名称'])#全模型
#data = result.drop(columns = ['O3','一氧化氮','一氧化碳','二氧化硫','氮氧化物','PM2.5','年','月','日','点位名称','工作日'])#有无工作日
...
for i in range(1,matrix.shape[0]):
    for j in range(0,i):
        ...
        pm_i = abs(matrix.ix[i,'二氧化氮'])
        pm_j = abs(matrix.ix['二氧化氮',j])
        if value>=0.7:
            index = rows[i]
            column = columns[j]
...
#            pm_i = abs(matrix.ix[heads[i],'二氧化氮'])
#            pm_j = abs(matrix.ix['二氧化氮',heads[j]])
#            if pm_i>pm_j:
...
x = result_scaled_minmax.drop(columns=['二氧化氮'], axis=1)
y = result['二氧化氮']
