...
with open('监测点信息汇总_新_300.csv', newline='',encoding='UTF-8-sig') as f:
...
################################个数#######################################
line_list = []
with open("公交站_500.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
number.rename(columns={0:'公交站_500'}, inplace=True)#重新命名
...
#data.drop(columns=['公交站_500_y'],inplace=True)

################################长度#########################################
line_list = []
with open("普通道路_500.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
#data = line_list[[2,14,10,11]]
#data.rename(columns={2:'点位名称', 14:'name', 10:'area', 11:'city'}, inplace=True)
line_list.rename(columns={2:'点位名称',10:'长度'}, inplace=True)#修改列名

for i,element in enumerate(line_list['长度']):#将长度改为浮点数
    line_list.loc[i,'长度'] = float(line_list.loc[i,'长度'])
    
number = line_list.groupby(['点位名称'])['长度'].sum().reset_index().sort_values(by=['点位名称'],ascending=True)#分组统计
number.rename(columns={'长度':'普通道路_500'}, inplace=True)#修改列名
...
################################人口#########################################
line_list = []
with open("人口密度_500.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
#data = line_list[[2,14,10,11]]
#data.rename(columns={2:'点位名称', 14:'name', 10:'area', 11:'city'}, inplace=True)
line_list.rename(columns={2:'点位名称',33:'人口'}, inplace=True)#修改列名
...
number.rename(columns={'人口':'人口密度_500'}, inplace=True)#修改列名
...
###################################面积########################################
line_list = []
with open("土地利用_500.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
