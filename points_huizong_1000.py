...
with open('监测点信息汇总_新_500.csv', newline='',encoding='UTF-8-sig') as f:
...
################################个数#######################################
line_list = []
with open("公交站_1000.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
number.rename(columns={0:'公交站_1000'}, inplace=True)#重新命名
...
number.rename(columns={'长度':'主要道路_1000'}, inplace=True)#修改列名
...
number.rename(columns={'人口':'人口密度_1000'}, inplace=True)#修改列名
...
###################################面积########################################
line_list = []
with open("土地利用_1000.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
