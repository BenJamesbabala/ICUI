...
################################################################################
workbook = xlrd.open_workbook(r'C:\Users\xinyisong\Desktop\ICUI\regression\日数据汇总.xlsx')
sheet = workbook.sheet_by_index(0) #读取第0个sheet表

rows_list = []

nrows = sheet.nrows #行数
ncols = sheet.ncols #列数
names = sheet.row_values(0) #从第一行获取列名

for i in range(1,nrows):
    rows_list.append(sheet.row_values(i)) #汇聚成列表，每个元素是原来sheet表中的一列
result = pd.DataFrame(rows_list,columns = names) #可以由列表直接转换为DataFrame，并指定列名   

result = result.iloc[:,12:result.shape[1]]#删除所有的月份变量
result = result.drop(columns = ['PM2.5','年','月','点位名称'])
...
x = result2.drop(columns=['二氧化氮'], axis=1)
y = result['二氧化氮']
