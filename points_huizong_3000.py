...
with open('监测点信息汇总_1000.csv', newline='',encoding='UTF-8-sig') as f:
...
################################个数#######################################
line_list = []
with open("加油站_3000.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
number.rename(columns={0:'加油站_3000'}, inplace=True)#重新命名
...
################################长度#########################################
line_list = []
with open("主要道路_3000.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
#data = line_list[[2,14,10,11]]
#data.rename(columns={2:'点位名称', 14:'name', 10:'area', 11:'city'}, inplace=True)
line_list.rename(columns={2:'点位名称',11:'长度'}, inplace=True)#修改列名
...
number.rename(columns={'长度':'主要道路_3000'}, inplace=True)#修改列名
...
################################人口#########################################
line_list = []
with open("人口密度_3000.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
number.rename(columns={'人口':'人口密度_3000'}, inplace=True)#修改列名
...
#data = line_list[[2,14,10,11]]
#data.rename(columns={2:'点位名称', 14:'name', 10:'area', 11:'city'}, inplace=True)
line_list.rename(columns={2:'点位名称',13:'类型',15:'面积'}, inplace=True)#修改列名
...
data = data.fillna(0)
####################################导出######################################
data.to_csv('监测点信息汇总_3000.csv',index = False,encoding = 'utf_8_sig') 