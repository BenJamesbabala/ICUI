...
################################读取大表格#####################################
wb=xlrd.open_workbook("监测点信息汇总_新.xlsx")
...
with open("加油站_100.txt", newline='',encoding='utf-8') as f:
    lines = f.readlines()
...
mm.rename(columns={0:'加油站_100'}, inplace=True)#重新命名
...
#data.drop(columns=['普通道路_100'], inplace=True)
...
#data = line_list[[2,14,10,11]]
#data.rename(columns={2:'点位名称', 14:'name', 10:'area', 11:'city'}, inplace=True)
line_list.rename(columns={10:'点位名称',7:'类型',14:'面积'}, inplace=True)#修改列名
...
####################################导出######################################
data.to_csv('监测点信息汇总_新_100.csv',index = False,encoding = 'utf_8_sig') 
