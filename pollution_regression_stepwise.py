...
###############################导入数据########################################
name_list = []
with open('周数据汇总.csv', newline='',encoding='UTF-8-sig') as f:
...
#data = result.drop(columns = ['二氧化氮','年','月','点位名称'])
#data = result.drop(columns = ['二氧化氮','年','月','日','点位名称'])
data = result.drop(columns = ['二氧化氮','点位名称'])
